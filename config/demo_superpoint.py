...
import sys
print(sys.version)
## solve cv2 problem related to ROS kinetic
...
class SuperPointFrontend(object):
  ...
  def run_2(self, img):
    start1 = time.time()
    pts,desc,_ = self.run(img)
    end1 = time.time()
    print("number of points: ",np.shape(pts)[1])
    print("forward pass computation time: ",end1-start1)
    return [pts,desc]

  # new function
  def nn_match_two_way(self, outputs_1, outputs_2):
    """
    ...
    matches[2, :] = scores
    end1 = time.time()
    print("number of matches: ",np.shape(matches)[1])
    print("match computation time: ",end1-start1)
    return matches
...
# new function
# use this fucntion to read images in C++ files
def read_original_image(impath):
    """ Read image as grayscale and resize to img_size.
    Inputs
      impath: Path to input image.
      img_size: (W, H) tuple specifying resize size.
    ...
    # cv2.namedWindow(win)
    # cv2.imshow(win, grayim)

    return grayim

# new function
# use this function to visualize point selection and matches in C++ files
def visualize_points_and_matches(outputs_1,outputs_2,matches,img_1,img_2):
    img_out = np.hstack((img_1,img_2))
    x_offset = np.shape(img_1)[1]
    
    # draw points in img_1
    pts_1 = outputs_1[0]
    for pt in pts_1.T:
      pt1 = (int(round(pt[0])), int(round(pt[1])))
      cv2.circle(img_out, pt1, 3, (0, 0, 255), -1, lineType=16)

    # draw points in img_2
    pts_2 = outputs_2[0]
    for pt in pts_2.T:
      pt2 = (int(round(pt[0])+x_offset), int(round(pt[1])))
      cv2.circle(img_out, pt2, 3, (0, 0, 255), -1, lineType=16)

    # draw matches
    for match in matches.T:
      pt1 = pts_1.T[int(match[0])]
      pt2 = pts_2.T[int(match[1])]
      pt1_xy = (int(round(pt1[0])), int(round(pt1[1])))
      pt2_xy = (int(round(pt2[0])+x_offset), int(round(pt2[1])))
      img_out = cv2.line(img_out, pt1_xy, pt2_xy, (255, 0, 0), 1)
...
if __name__ == '__main__':
  ...
  while True:
    ...
    print("number of points: ",np.shape(pts))
    end1 = time.time()
    print("computation time: ",end1-start1)
